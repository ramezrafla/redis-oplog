...
export const onMutation = (collectionName, _id, fields) => {
  ...
  if (item) {
    ...
    // check for fields collision
      ...
      if (common.length && fieldsToSkip[collectionName]) common = _.without(common, ...fieldsToSkip[collectionName])
      if (common.length == 0) {
        ...
        item.timer = Meteor.setTimeout(() => onTimer(uid), 100 + Math.random()*250)
        item.updatedAt = now
      }
    }
...
export const onForce = (collectionName, _id) => {
  if (skip) return
  const uid = collectionName + '-' + _id
  debug('[RaceConditionsManager - onForce] ' + uid)
  const item = history[uid]
  if (item) {
...
export const init = () => {
  ...
  fieldsToSkip = Config.raceDetectionFieldsToIgnore || { }
