...
export const onMutation = (collectionName, _id, fields, doc) => {
  ...
  if (item) {
    ...
    // check for fields collision
      ...
      if (common.length && differentObjects(item.doc, doc, common)) {
...
function differentObjects(doc1, doc2, fields) {
  var l = fields.length
  for (var i = 0; i < l; i++) {
...
// taken from ClassroomAPP /lib/Utils -- output negated, returns true if fields are different
function compare(a,b) {
  if (a === b) return false
  const typeA = typeof a
  // eslint-disable-next-line
  if (typeA !== typeof b) return true
  // object / array vs regular
  if (['number','string','boolean'].includes(typeA)) return true
  if (_.isEmpty(a) && _.isEmpty(b)) return false
  return true // we don't do a deep diff on data, so assume they are different
}
