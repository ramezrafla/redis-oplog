...
export const onMutation = (collectionName, _id, fields, doc) => {
  ...
  if (item) {
    ...
    // check for fields collision
      ...
      if (common.length && areDifferentObjects(item.doc, doc, common)) {
        console.log('Redis-Oplog: Handling potential race condition on ' + uid + ': ' + common.join(', '))
    ...
    // either we have field collision or the doc was removed
      ...
      console.log('Redis-Oplog: Handling potential race condition on ' + uid + (fields ? ': ' + fields.join(', ') : ''))
...
// returns true of the docs are different
function areDifferentObjects(doc1, doc2, fields) {
  var l = fields.length
  for (var i = 0; i < l; i++) {
    const field = fields[i]
    if (deepDiff(doc1[field], doc2[field])) return true
  }
}
