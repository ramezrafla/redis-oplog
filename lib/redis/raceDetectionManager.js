...
export const onMutation = (collectionName, _id, fields, doc) => {
  ...
  if (item) {
    ...
    // check for fields collision
      ...
      if (common.length && DeepDiff(_.pick(item.doc, common), _.pick(doc, common))) {
        console.log('Redis-Oplog: Handling potential field collision race condition on ' + uid + ': ' + common.join(', '))
