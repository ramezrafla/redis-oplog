...
function listenAll(cursorDescription, listenCallback) {
  ...
  return {
    stop: function() { _.each(listeners, function(listener) { listener.stop() }) }
  }
}
...
function forEachTrigger(cursorDescription, triggerCallback) {
  ...
  if (specificIds) {
    _.each(specificIds, function(id) { triggerCallback(_.extend({ id }, key)) })
...
export default function PollingObserveDriver(options) {
  ...
  if (options._testOnlyPollCallback) {
    ...
    self._stopCallbacks.push(function() { Meteor.clearInterval(intervalHandle) })
...
_.extend(PollingObserveDriver.prototype, {
  ...
  _unthrottledEnsurePollIsScheduled: function() {
    ...
    self._taskQueue.queueTask(function() { self._pollMongo() })
  },
  ...
  _suspendPolling: function() {
    ...
    if (self._pollsScheduledButNotStarted !== 1)
      throw new Error('_pollsScheduledButNotStarted is ' + self._pollsScheduledButNotStarted)
  },
  ...
  _resumePolling: function() {
    ...
    self._taskQueue.runTask(function() { self._pollMongo() })
  },
  ...
  stop: function() {
    ...
    _.each(self._stopCallbacks, function(c) { c() }) // Release any write fences that are waiting on us.

    _.each(self._pendingWrites, function(w) { w.committed() })
