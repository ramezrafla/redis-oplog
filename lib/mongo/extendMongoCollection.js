  ...
  _.extend(proto, {
    ...
    startCaching(timeout) {
      ...
      this.direct = {
        ...
        upsert: (selector, modifier, config, callback) => proto.upsert.call(this, selector, modifier, config, callback),
      }
        ...
        upsert: (func) => {
          if (!this._beforeUpsertHooks) {
            ...
            this.upsert = (selector, modifier, config, callback) => {
              ...
              const userId = getUserId()
              const fields = getFields(modifier)
              this._beforeUpsertHooks.forEach((func) => docs.forEach((doc) => func.call(null, userId, doc, fields, modifier, config)))
              const result = this.direct.upsert(selector, modifier, config, callback)
              docs.forEach((previous) => {
                const after = this.direct.findOne(previous._id)
                this._afterUsertHooks.forEach((func) => func.call({ previous }, userId, after, fields, modifier, config))
              })
              return result
            }
          }
        ...
        upsert: (func) => {
          if (!this._beforeUpsertHooks) {
            ...
            this.upsert = (selector, modifier, config, callback) => {
              ...
              docs.forEach((previous) => {
                const after = this.direct.findOne(previous._id)
                this._afterUpsetHooks.forEach((func) => func.call({ previous }, userId, after, fields, modifier, config))
              })
              return result
            }
          }
