...
const DEBUG_QUERIES = (Meteor.isDevelopment || Meteor.isStaging) && false
...
export default () => {
  ...
  _.extend(proto, {
    ...
    /**
         ...
         */
    insert(doc, config) {
      DEBUG_QUERIES && console.log(`[insert ${this._name}]`, doc, config);
    ...
    update(selector, modifier, config, callback) {
      DEBUG_QUERIES && console.log(`[update ${this._name}]`, selector, modifier, config);
    ...
    remove(selector, config) {
      DEBUG_QUERIES && console.log(`[remove ${this._name}]`, selector, config);
    ...
    setCache(doc) {
      ...
      DEBUG_CACHE && console.log("[redis-oplog] setCache", this._name, id, Object.keys(doc));
    ...
    getCache(id) {
      ...
      if (doc) {
        this._lastAccess.set(id, getTime())
        ++this._hits
        DEBUG_CACHE && console.log("[redis-oplog] hitCache", this._name, id, Object.keys(doc));
    ...
    startCaching(timeout) {
      ...
      const findOne = (selector = {}, options = {}) => {
        var doc
        var id
        const { transform = null } = options

        DEBUG_QUERIES && console.log(`[findOne ${this._name}]`, selector, options);
      ...
      const find = (selector = {}, options = {}) => {
        ...
        DEBUG_QUERIES && console.log(`[find ${this._name}]`, selector, options);
          ...
          if (options.skipSave) overrideCB = (cb, _this) => {
            ...
            return (doc) => {
              ...
              if (this.hasCache(doc._id)) {
                ...
                if (doc && transform) doc = transform(doc)
    ...
    fetchInCacheFirst(ids, options = {}) {
      ...
      const notFoundIds = []
      ids.forEach((id) => {
        let doc = this.getCache(id)
        if (transform && doc) doc = transform(doc)
      ...
      if (options?.fields) {
        try {
          const projector = LocalCollection._compileProjection(options.fields)
          return result.map((doc) => transform ? projector(transform(doc)) : projector(doc))
        }
        catch(e) { }
      }
      if (transform) {
