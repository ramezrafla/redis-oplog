...
export default () => {
  ...
  _.extend(proto, {
    ...
    startCaching(timeout) {
      ...
      const findOne = function(selector = {}, options) {
        var doc
        var id
      ...
      this.direct = {
        find: (selector, options) => Originals.find.call(this, selector, options),
        findOne,
        ...
        update: (func) => {
          if (!this._beforeUpdateHooks) {
            ...
            this.update = (selector, modifier, config, callback) => {
              ...
              _.each(this._afterUpdateHooks, (func) => docs.forEach((doc) => func.call({previous:doc}, userId, this.direct.findOne(doc._id), fields, modifier, config)))
              return result
            }
          }
