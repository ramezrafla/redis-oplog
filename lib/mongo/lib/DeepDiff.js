...
const DeepDiff = function() {
  ...
  function observableDiff(lhs, rhs, observer, prefilter, orderIndependent) {
    ...
    if (observer) {
      for (var i = 0; i < changes.length; ++i) observer(changes[i])
    }
    return changes
  }
  ...
  function applyChange(target, source, change) {
    ...
    if (target && change && change.kind) {
      ...
      while (++i < last) {
        if (typeof it[change.path[i]] === 'undefined') it[change.path[i]] = (typeof change.path[i + 1] !== 'undefined' && typeof change.path[i + 1] === 'number') ? [] : {}
        it = it[change.path[i]]
      }
      switch (change.kind) {
        case 'A':
          if (change.path && typeof it[change.path[i]] === 'undefined') it[change.path[i]] = []
  ...
  function revertChange(target, source, change) {
    if (target && source && change && change.kind) {
      ...
      for (i = 0; i < u; i++) {
        if (typeof it[change.path[i]] === 'undefined') it[change.path[i]] = {}
        it = it[change.path[i]]
      }
      switch (change.kind) {
        case 'A':
