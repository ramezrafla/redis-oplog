...
export default class Mutator {
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    try {
      ...
      // we can't send $unset so we have to modify the doc in a way our redis listener can interpret as $unset
      var hasUnset = false
      var skipRedis = this._skipRedis || !config.pushToRedis
      const unsetKeys = []

      // if we are skipping redis this would waste CPU cycles
      if (!skipRedis && modifier.$unset) {
        hasUnset = true
        if (!modifier.$set) modifier.$set = { }
        _.forEach(modifier.$unset, (val, key) => {
          unsetKeys.push(key)
          modifier.$set[key] = '$$undefined$$'
        })
      ...
      docs.forEach((before, index) => {
        ...
        if (diff) {
          if (hasUnset) this.setCache(_.omit(after, unsetKeys))
          else this.setCache(after)
      ...
      if (skipRedis || docs.length == 0) return result
