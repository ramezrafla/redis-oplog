...
export default class Mutator {
  static insert(Originals, data, _config) {
    const config = getMutationConfig(this, _config, { doc: data, event: Events.INSERT })
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    const config = getMutationConfig(this, _config, { event: Events.UPDATE, selector, modifier })
    ...
    if (!docIds) {
      // we get all the fields so we can update our local cache
    ...
    if (config.upsert) return Mutator._handleUpsert.call(
      this,
      Originals,
      selector,
      modifier,
    ...
    try {
      const result = Originals.update.call(
        this,
        {_id: { $in: docIds } }, // we already got the IDs, let's speed up this call
        modifier,
        config
      )
      ...
      if (this._skipRedis || !config.pushToRedis) return result
    ...
    try {
      ...
      if (data.insertedId) {
        ...
        if (modifier.$set && typeof selector == 'object') {
          doc = { ...selector, ...modifier.$set, _id:data.insertedId }
        }
        // we shouldn't be here -- fallback just in case
        else {
          docs = this.find(data.insertedId).fetch()
        ...
        if (callback) runCallbackInBackground(() => callback.call(this, null, data))
      ...
      // updated
        ...
        if (docIds.length === 0 || data.numberAffected !== docIds.length) {
          ...
          // nothing to do
          if (this._skipRedis || !config.pushToRedis || docs.length == 0) return data
  ...
  static remove(Originals, selector, _config) {
    ...
    const config = getMutationConfig(this, _config, { selector, event: Events.REMOVE })
