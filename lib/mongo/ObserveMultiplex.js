...
export function ObserveMultiplexer(options) {
  ...
  self._cache = new LocalCollection._CachingChangeObserver({ ordered: false }) // purely for APM otherwise we get an error
  ...
  _.each(self.callbackNames(), function(callbackName) {
    self[callbackName] = function(/* ... */) { self._applyCallback(callbackName, _.toArray(arguments)) }
  })
}
...
_.extend(ObserveMultiplexer.prototype, {
  ...
  _applyCallback: function(callbackName, args) {
    ...
    this._queue[runType](function() {
      ...
      if (callbackName == 'added') {
        ...
        console.error('ObserveMultiplex: Unknown callback ' + callbackName)
      }

      // If we haven't finished the initial adds, then we should only be getting adds
      if (!self._ready() && callbackName !== 'added') throw new Error('Got ' + callbackName + ' during initial adds')
  ...
  _sendAdds: function(handle) {
    ...
    self._ids.forEach(function(id) {
      ...
      var fields = self._collection.findOne(id)
      if (!fields) return
      if (self.projector) {
        try { fields = self.projector(fields) }
        catch(e) { }
      }
      add(id, fields)
    })
...
export function ObserveHandle(multiplexer, callbacks) {
  ...
  _.each(multiplexer.callbackNames(), function(name) { if (callbacks[name]) self['_' + name] = callbacks[name] })
  self._stopped = false
