...
export default class Mutator {
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    try {
      ...
      var cleared = modifier.$unset && _.keys(modifier.$unset) || []
      docs.forEach((before, index) => {
        ...
        if (this._skipDiff || config.skipDiff) {
          ...
          if (diff) {
            this.setCache(after)
            if (diff[0].length) docs[index] = _.pick(after, [...diff[0], '_id'])
            if (diff[1].length) cleared = _.union(cleared, diff[1])
            // console.log(diff)
          }
  ...
  static remove(Originals, selector, _config) {
    ...
    var docs = this.find(selector, { transform: null, skipSave:true }).fetch()
    var docIds = docs.map((doc) => doc._id)
