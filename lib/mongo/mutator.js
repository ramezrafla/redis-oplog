...
export default class Mutator {
  static insert(Originals, data, _config) {
    ...
    try {
      var docId
      if (config.skipDB) {
        docId = data._id || Random.id()
      }
      else {
        docId = Originals.insert.call(this, data)
      }
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    try {
      ...
      dispatchUpdate(
        ...
        docs,
        topLevelFields,
        modifier.$unset && _.keys(modifier.$unset)
      )
    ...
    try {
      ...
      if (result.insertedId) {
        ...
        // prone to race conditions in secondary-read environment
          ...
          doc = Originals.findOne(result.insertedId)
        }
  ...
  static remove(Originals, selector, _config) {
    ...
    if (!docIds) {
      ...
      docs = this.find(selector, { fields: { _id: 1 }, transform: null, skipCache:true }).fetch()
      docIds = docs.map((doc) => doc._id)
    }
